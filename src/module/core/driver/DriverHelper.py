...
class DriverHelper(ChromiumDriver):
    ...
    # 初始打开谷歌浏览器
    def __init__(self, job_name, options, windows_size=1, headless=False, mute=False, image=True, infobar=False):
        """
        初始化浏览器句柄
        :param job_name:    任务名称
        ...
        """根据输入的参数,获取装配好的选项"""
        options = self.__prepare_befor(options, headless, mute, image, infobar)
    ...
    def __prepare_befor(self, options, headless, mute, image, infobars):
        """
        基础配置
        :param options: 浏览器初始化选项
        :param headless: 禁默开关
